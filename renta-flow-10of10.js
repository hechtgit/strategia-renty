  ...
  const apply=()=>{
    ...
    if(delivery){
      ...
      if(gate){
        ...
        for(const child of children)(child===title||child===label?left:right).appendChild(child);
      }
    }
    ...
    if(consult){
      ...
      body.innerHTML="Vďaka tomuto komplexnému pohľadu získate realistický pohľad na svoje súčasné portfólio, jeho výnosový potenciál, riziko, likviditu a schopnosť financovať plánovanú rentu a jednotlivé investičné produkty <strong>prestanú byť samotným cieľom a stanú sa nástrojmi vašej stratégie</strong>.";
