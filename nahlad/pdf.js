...
(function () {
  ...
  function zoStranky() {
    ...
    return {
      ...
      suhrn: suhrn,
      odolnost: (function () {
        var sek = document.getElementById("odolnost");
        if (!sek || sek.hidden) return null;
        return {
          nadpis: text(sek.querySelector("h2")),
  ...
  function vytvor(d) {
    ...
    if (d.odolnost && (d.odolnost.riadky.length || d.odolnost.uvod)) {
      ...
      medzera(1.5);
      odstavec(d.odolnost.uvod, 9, "normal", SEDA, SIRKA, 1.45);
      medzera(4);

      d.odolnost.riadky.forEach(function (r, i) {
        if (i) {
          doc.setDrawColor(LINKA[0], LINKA[1], LINKA[2]);
          doc.setLineWidth(0.2);
          doc.line(OKRAJ, y, OKRAJ + SIRKA, y);
        ...
        y += i ? 5.5 : 0;
        doc.setFont("Asap", i ? "normal" : "bold");
        doc.setFontSize(9.5);
        doc.setTextColor(i ? SEDA[0] : TMAVA[0], i ? SEDA[1] : TMAVA[1], i ? SEDA[2] : TMAVA[2]);
        doc.text(doc.splitTextToSize(r[0], SIRKA - 55), OKRAJ, y);
        doc.setFont("Asap", "bold");
        doc.setFontSize(12);
        doc.setTextColor(ZLATA[0], ZLATA[1], ZLATA[2]);
        doc.text(r[1], OKRAJ + SIRKA, y, { align: "right" });
        y += 6.5;
      });
      ...
      /* bez testu odolnosti zostáva výzva na prvej strane */
      nadpisSTelom(d.dalejNadpis, d.dalej, 11, 8.5);
    }
