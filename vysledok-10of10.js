...
(()=>{
  ...
  /* HAVE_RENT_PRESENTATION:START */
  function haveRentPresentation({now,start,end,rentToday,infl,inflOn,pension,firstPayment}){
    const years=Math.max(0,start-now);
    const hasInflation=!!inflOn&&infl>0;
    const nominalPayment=Number.isFinite(firstPayment)
      ?firstPayment:(hasInflation?rentToday*Math.pow(1+infl/100,years):rentToday);
    const yrs=n=>`${n} ${n===1?'rok':(n>=2&&n<=4?'roky':'rokov')}`;
    const duration=pension==='perpetuity'?'bez časového obmedzenia':`počas ${yrs(Math.max(0,end-start))}`;
    return {
      hasInflation,firstPayment:nominalPayment,
      label:hasInflation?'Mesačná renta v dnešných cenách':'Mesačná renta',
      sub:hasInflation?`Prvá výplata vo veku ${start} rokov: ${fmt(nominalPayment)} mesačne.`:'mesačne',
      goal:hasInflation
        ?`Váš majetok môže od ${start} rokov priniesť privátnu rentu <strong>${fmt(rentToday)} mesačne</strong> v dnešnej hodnote ${duration}. Prvá výplata vo veku ${start} rokov je približne <strong>${fmt(nominalPayment)} mesačne</strong>.`
        :`Váš majetok môže od ${start} rokov priniesť privátnu rentu <strong>${fmt(rentToday)} mesačne</strong> ${duration}.`
    };
  }
  ...
  function correctHaveRent(){
    if(q.get('sit')!=='have'||q.get('goal')!=='rent')return;
    const params={
      now:Number(q.get('now')),start:Number(q.get('start')),end:Number(q.get('end')),
      rentToday:Number.NaN,infl:Number(q.get('infl')),
      inflOn:q.get('infl_on')==='1',pension:q.get('pension')
    };
    const label=document.getElementById('s-label2'),value=document.getElementById('s-value2'),
          sub=document.getElementById('s-sub'),goal=document.getElementById('ciel');
    ...
    if(value)value.textContent=fmt(params.rentToday);
    if(sub)sub.textContent=copy.sub;
    if(goal)goal.innerHTML=copy.goal;
  }
  ...
  function explainHistory(){
    ...
    sensitivity.innerHTML='<summary>Doplňujúci detail: ako sa výsledok mení s vyššou rezervou</summary><p class="uvod">'+sensitivityIntro+'</p>'+
  ...
  function apply(){correctHaveRent();staticCopy();exceptions();explainHistory();summary();assumptions();}
